...
def sanitize_text(text):
    """Remove invalid surrogate characters from text to prevent UnicodeEncodeError"""
    if not text:
        return ""
    # Remove lone surrogate characters (U+D800 to U+DFFF)
...
async def progress_bar(current, total, reply, start):
    if timer.can_send():
        ...
        if diff < 1:
            ...
            try:
                await reply.edit(f'<blockquote>`╭──⌯═════Uploading══════⌯──╮\n├⚡ {progress_bar}\n├⚙️ Progress ➤ | {perc} |\n├🚀 Speed ➤ | {sp} |\n├📟 Processed ➤ | {cur} |\n├🧲 Size ➤ | {tot} |\n├🕑 ETA ➤ | {eta} |\n╰─═══✨🦋{CREDIT}🦋✨═══─╯`</blockquote>')
